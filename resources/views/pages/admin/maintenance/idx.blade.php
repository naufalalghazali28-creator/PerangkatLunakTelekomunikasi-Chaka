...
new class extends Component {
    ...
    public function exportExcel() {
        return Excel::download(new class implements \Maatwebsite\Excel\Concerns\FromQuery, \Maatwebsite\Excel\Concerns\WithHeadings, \Maatwebsite\Excel\Concerns\WithMapping {
            public function query() {
                return User::where('role', 'maintenance')->where(function($query) {
    ...
    public function exportPdf() {
        $users = User::where('role', 'maintenance')->where(function($query) {
            $query->where('email', 'like', '%@staff.id')
                  ->orWhere('email', 'like', '%@bems.id');
        })->get();
        ...
        return response()->streamDownload(function () use ($pdf) {
            echo $pdf->output();
        }, 'Data_Maintenance_' . now()->format('d-m-Y') . '.pdf');
    }

    public function render() {
        return <<<'HTML'
        <div>
            <x-header title="Admin | Maintenance Management" subtitle="Teknisi pendaftar sensor" separator progress-indicator>
                <x-slot:actions>
                    ...
                    <x-button label="Add Maintenance" icon="o-plus" wire:click="$set('staffModal', false)" class="btn-primary btn-sm" />
                </x-slot:actions>
            </x-header>
            ...
            <x-card shadow>
                ...
                <x-table :headers="$this->headers" :rows="\App\Models\User::where('role', 'maintenance')
                ...
                @endscope
                    @scope('actions', $user)
                        <div class="flex gap-2">
                            ...
                            <x-button icon="o-trash" wire:click="deleteStaff({{ $user->id }})" wire:confirm="Hapus teknisi ini?" class="btn-ghost btn-sm text-error" />
            ...
            <x-modal wire:model="staffModal" title="Tambah Teknisi Baru" separator>
                <x-input label="Nama Lengkap" wire:model="staffName" hint="Otomatis jadi @staff.id" />
                <x-slot:actions>
