...
new class extends Component {
    ...
    // Chart: Stacked bar semua sensor aktif
    #[Computed]
    public function allSensorData()
    {
        ...
        $colors   = ['#3b82f6','#f59e0b','#ef4444','#22c55e','#8b5cf6','#14b8a6','#f43f5e','#0ea5e9'];
        $labels   = $nodes->map(fn($n) => substr($n->name,0,12).(strlen($n->name)>12?'…':''))->toArray();
        $datasets = [];
        ...
        foreach ($nodes as $idx => $node) {
            ...
            $data        = array_fill(0, count($nodes), 0);
            $data[$idx]  = $value;
            $datasets[] = [
    ...
    // Chart: Line chart satu sensor terpilih
    #[Computed]
...
<div
    x-data="{
        charts: {},
        init() { this.$nextTick(() => { this.destroyAll(); this.buildAll(); }); },
        ...
        buildAll() { this.buildType(); this.buildStacked(); this.buildSingle(); },

        buildType() {
            const el = document.getElementById('vChartType'); if(!el) return;
            const d  = {{ Js::from($this->nodeTypeChartData) }};
            this.charts.type = new Chart(el, { type:'doughnut', data:{
                labels: d.labels,
                datasets:[{data:d.data,backgroundColor:['#3b82f6','#f59e0b','#ef4444','#22c55e'],borderWidth:2,borderColor:this.isDark()?'#18181b':'#fff'}]
            }, options:{responsive:true,plugins:{legend:{position:'bottom',labels:{color:this.lbl(),font:{size:11}}}},cutout:'62%'}});
        },

        buildStacked() {
            const el = document.getElementById('vChartStacked'); if(!el) return;
            const d  = {{ Js::from($this->allSensorData) }};
            if(!d||!d.datasets||!d.datasets.length) return;
            this.charts.stacked = new Chart(el, { type:'bar', data:{labels:d.labels,datasets:d.datasets},
                options:{responsive:true,interaction:{mode:'index',intersect:false},
                    plugins:{legend:{labels:{color:this.lbl(),font:{size:10}}}},
                    scales:{x:{stacked:true,ticks:{color:this.lbl()},grid:{display:false}},
                            y:{stacked:true,ticks:{color:this.lbl()},grid:{color:this.grid()},beginAtZero:true}}}});
        },

        buildSingle() {
            const el = document.getElementById('vChartSingle'); if(!el) return;
            const d  = {{ Js::from($this->singleSensorData) }};
            if(!d||!d.labels||!d.labels.length) return;
            this.charts.single = new Chart(el, { type:'line', data:{
                labels: d.labels,
                datasets:[{label:d.nodeName+' ('+d.unit+')',data:d.data,borderColor:'#8b5cf6',backgroundColor:'rgba(139,92,246,0.08)',tension:0.4,fill:true,pointRadius:4,pointHoverRadius:6}]
            }, options:{responsive:true,interaction:{mode:'index',intersect:false},
                plugins:{legend:{labels:{color:this.lbl(),font:{size:11}}}},
                scales:{x:{ticks:{color:this.lbl()},grid:{color:this.grid()}},
...
>
    ...
    <div class="grid grid-cols-5 gap-4 mb-4">
        <div class="col-span-2">
            <x-card title="Distribusi Tipe Sensor" shadow
                ...
                <div class="flex justify-center py-2" style="min-height:200px">
                    <canvas id="vChartType" style="max-height:200px"></canvas>
                </div>
            </x-card>
        </div>
    ...
    {{-- ROW 2: Stacked Bar semua sensor --}}
    <x-card title="Data Semua Sensor Aktif" subtitle="Nilai dummy terkini — tiap sensor sebagai stack terpisah"
        shadow class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl mb-4">
        <canvas id="vChartStacked" style="max-height:180px"></canvas>
    </x-card>
    ...
    <x-card shadow class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl mb-4">
        ...
        <canvas id="vChartSingle" style="max-height:200px"></canvas>
        @else
