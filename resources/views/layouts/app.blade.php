...
<head>
    ...
    <style>
        ...
        /* Sidebar nav item aktif per role */
...
>
    ...
    <nav class="flex-1 overflow-y-auto no-scrollbar px-2 py-3 space-y-0.5">
        ...
        @php
            $adminSections = [
                ['label' => 'Overview',      'icon' => 'o-squares-2x2',      'route' => 'admin',            'url' => '/admin'],
                ['label' => 'Clients',       'icon' => 'o-building-office-2','route' => 'admin.client',     'url' => '/admin/client'],
                ['label' => 'Gedung',        'icon' => 'o-building-office',  'route' => 'admin.gedung',     'url' => '/admin/gedung'],
            ];
            $staffSections = [
        ...
        <div x-data="{ staffOpen: {{ $staffInitOpen }} }">
            ...
            >
                ...
                {{-- Gunakan SVG langsung agar Alpine binding :class bekerja --}}
            ...
            >
                ...
                <a href="{{ url($s['url']) }}"
                   class="flex items-center gap-3 px-3 py-2 rounded-xl transition-all text-sm
                          {{ $on($s['match']) ? 'nav-active-green' : 'nav-idle' }}"
                >
        ...
        <a href="{{ url($s['url']) }}"
           class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group relative nav-idle"
           :class="!open && 'justify-center px-0'"
        >
            <x-icon name="{{ $s['icon'] }}" class="w-5 h-5 shrink-0" />
            <span x-show="open" x-cloak class="text-sm font-medium">{{ $s['label'] }}</span>
        ...
        @php
            $maintNav = [
                ...
                [
                    'label' => 'Daftar Node',
                    'icon'  => 'o-plus-circle',
                    'url'   => '/maintenance/register-node',
                    'match' => 'maintenance/register-node*',
                    'desc'  => 'Tambah node baru ke gedung'
                ],
                [
                    'label' => 'Node Inventory',
                    'icon'  => 'o-server-stack',
                    'url'   => '/maintenance/nodes',
                    'match' => 'maintenance/nodes*',
                    'desc'  => 'Semua node terdaftar'
                ],
                [
                    'label' => 'Log Instalasi',
                    ...
                    'desc'  => 'Riwayat pemasangan'
                ],
                [
                    'label' => 'Info Akun',
                    'icon'  => 'o-identification',
                    'url'   => '/maintenance/akun',
                    'match' => 'maintenance/akun*',
        ...
        {{-- Section label --}}
        <div x-show="open" x-cloak class="px-3 pt-3 pb-1 text-[10px] font-semibold text-zinc-600 uppercase tracking-[0.18em]">
            Maintenance
        </div>
        ...
        >
            ...
            {{-- Tooltip --}}
            <div x-show="!open" class="absolute left-full ml-3 px-2.5 py-1.5 rounded-lg bg-zinc-800 text-xs text-white whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity z-50">
                {{ $s['label'] }}
            </div>
        </a>
        @endforeach
        ...
        @php
            $opNav = [
                ...
                [
                    ...
                    'match' => 'operator/control*',
                    'desc'  => 'Aktifkan / nonaktifkan sensor'
                ],
                [
                    'label' => 'Live Monitor',
                    'icon'  => 'o-signal',
                    'url'   => '/operator/monitor',
                    'match' => 'operator/monitor*',
                    'desc'  => 'Data real-time sensor'
                ],
                [
                    'label' => 'Info Akun',
                    'icon'  => 'o-identification',
        ...
        @php
            $viewNav = [
                ...
                [
                    ...
                    'url'   => '/viewer/gedung',
                    'match' => 'viewer/gedung*',
                    'desc'  => 'Daftar gedung terdaftar'
                ],
                [
                    'label' => 'List Sensor',
                    'icon'  => 'o-eye',
                    'url'   => '/viewer/sensors',
                    'match' => 'viewer/sensors*',
                    'desc'  => 'Semua sensor & datanya'
                ],
                [
                    'label' => 'Info Akun',
                    'icon'  => 'o-identification',
            ...
            <div x-show="open" x-cloak>
                <form method="POST" action="/logout">
                    ...
                    <button type="submit" title="Logout"
                        class="w-8 h-8 rounded-lg hover:bg-zinc-900 flex items-center justify-center text-zinc-500 hover:text-red-400 transition-all">
...
<div class="flex-1 flex flex-col overflow-hidden">
    ...
    <header class="h-16 shrink-0 border-b border-zinc-200 dark:border-zinc-900 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-xl px-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            ...
            <button
                @click="dark = !dark"
                class="w-9 h-9 rounded-xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 flex items-center justify-center transition-all duration-200"
                :title="dark ? 'Switch to light mode' : 'Switch to dark mode'"
            >
                {{-- Sun icon (tampil saat dark mode aktif) --}}
                <svg x-show="dark" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707M17.657 17.657l-.707-.707M6.343 6.343l-.707-.707M12 8a4 4 0 100 8 4 4 0 000-8z" />
                </svg>
                {{-- Moon icon (tampil saat light mode) --}}
                <svg x-show="!dark" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
